...
func (self *Client) Read() {
	...
	for {
		...
		s := [][]byte{head, buf}
		full_buf := bytes.Join(s, []byte(""))
		self.TcpConn.Write(full_buf)
	}
}
...
func (self *Client) ReadTcp() {
	...
	for {
		...
		if _, err := io.ReadFull(self.TcpConn, head[:]); err != nil {
			self.Release()
			num = num - 1
			// only
			log.Print("other tcp connect err ", err)
		...
		if _, err := io.ReadFull(self.TcpConn, packet); err != nil {
			self.Release()
			num = num - 1
			// only
			log.Print("other tcp connect err ", err)
		...
		//log.Print("recv from other tcp : ", string(packet[:size]))
...
func main() {
	...
	http.ListenAndServe(":"+string(os.Args[3]), nil)
}
